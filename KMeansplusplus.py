...
# function of initiating centroids for the first iteration
# input: dataset matrix, number of clusters
# output: array of k centroids
def initiateClusters(dataset, k):
    # Randomly select a data point as the first centroids
    np.random.seed(12)
    # Get data points as centroids based on a randomly chosen index
    centroids = [dataset[np.random.randint(dataset.shape[0])]]
    # choose left centroids
    for _ in range(1, k):
        # Calculate the distance of each data point to the nearest centroids
        distances = np.array([min([np.linalg.norm(point - centroid) for centroid in centroids]) for point in dataset])

        # Calculate distance weight
        weights = distances / distances.sum()

        # Randomly select the next centroids based on weight
        new_centroid_index = np.random.choice(range(len(dataset)), p=weights)
...
def KMeansplusplus(dataset, k, maxIter=10):
    labels = None
...
centroids = initiateClusters(dataset, 3)
...
for i in range(1, 10):
    labels = KMeansplusplus(dataset, i)  # get labels for the data points
